...
var _ = Describe("Backends", func() {
	...
	Describe("Concurrent operations", func() {
		It("do not result in a race", func() {
			...
			go func() {
				<-readySetGo
				backends.SetState(backend, true)
				close(doneChans[2])
			}()

			go func() {
				<-readySetGo
				backends.SetState(backend, false)
				close(doneChans[3])
			}()
	...
	Describe("SetState (unhealthy)", func() {
		...
		Context("when this is active", func() {
			...
			It("sets another healthy backend as the new active backend", func() {
				...
				for _ = range backends.Healthy() {
					...
					backends.SetState(previousActive, false)
